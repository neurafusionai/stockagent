...
class Agent:
    def __init__(self, i, stock_a_price, stock_b_price, secretary, model):
        ...
        #self.stock_b_amount = 0  # Stocks are stored in lots, one lot = 10 shares, and the stock price is actually the price of one lot
        self.init_proper = self.get_total_proper(stock_a_price, stock_b_price)  # Initial Assets Subsequent borrowing shall not exceed the initial assets
    ...
    def loan_repayment(self, date):
        if self.quit:
            return
        # Check if the loan repayment date is approaching, repayment date is due, or if there is a bankruptcy investigation
        for loan in self.loans[:]:
