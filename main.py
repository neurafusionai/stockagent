        ...
        if action["action_type"] == "buy":
            ...
            for buy_action in stock_deals["buy"][:]:
                if action["price"] == buy_action["price"]:
                    ...
                    if action["amount"] > close_amount:  # Sell orders not yet closed, buy orders closed, the cycle continues.
                        log.logger.info(f"ACTION - BUY:{buy_action['agent']}, SELL:{action['agent']}, "
                                        f"STOCK:{stock.name}, PRICE:{action['price']}, AMOUNT:{close_amount}")
                        stock_deals["buy"].remove(buy_action)
                        action["amount"] -= close_amount
                    else:  # Buy order not yet completed, sell order completed
                        log.logger.info(f"ACTION - BUY:{buy_action['agent']}, SELL:{action['agent']}, "
                                        f"STOCK:{stock.name}, PRICE:{action['price']}, AMOUNT:{close_amount}")
...
def simulation(args):
    ...
    for date in range(1, util.TOTAL_DATE + 1):
        ...
        # Except for the issuance of b, delete all transactions from the previous day
        ...
        for agent in all_agents[:]:
            agent.chat_history.clear()  # Only save chat history for the current day
        ...
        for session in range(1, util.TOTAL_SESSION + 1):
            ...
            # Randomly define the transaction order
            sequence = list(range(len(all_agents)))
            random.shuffle(sequence)
            for i in sequence:
                agent = all_agents[i]
                # if agent.is_bankrupt:  Trading is suspended on days when cash < 0, and shares are sold after the trading session ends.
                #     continue
        ...
        # Trading day ends, forum information updated
