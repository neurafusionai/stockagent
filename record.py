...
existing_df = pd.DataFrame(columns=["trading day", "trading session", "stock type", "buying trader", "selling trader", "trading quantity", "trading price"])

# Merge new transaction records into an existing DataFrame
...
# Write the list of transaction records to an Excel file (create the file if it does not exist)
...
existing_df = pd.DataFrame(columns=["trading day", "trading phase", "price of stock A after phase", "price of stock B after phase"])

# Merge new trading records into an existing DataFrame
...
self.loan_amount = 0 
if self.if_loan == "yes": 
self.loan_type = loan_json["loan_type"] self.loan_amount = loan_json["amount"] 
self.will_loan = "no" 
self.will_buy_a = "no" 
self.will_sell_a = "no" 
self.will_buy_b = "no" 
...
existing_df = pd.DataFrame(columns=["Trader", "Trading Day", "Loan Status", "Loan Type", "Loan Amount",

"Loan Status Tomorrow", "Buy A Tomorrow", "Sell A Tomorrow", "Buy B Tomorrow", "Sell B Tomorrow"])
...
existing_df = pd.DataFrame(columns=["Trader", "Trading Day", "Trading Stage", "Total Assets Before Trading",

"Cash Held Before Trading", "Value of A-Shares Held Before Trading", "Value of B-Shares Held Before Trading",

"Order Type", "Order Stock Category", "Order Quantity", "Order Price"])
